...
$(document).ready(function() {
  $('#discountForm').submit(function(event) {
    ...
    $.post('/discount', { discountCode: discountCode })
      .done(function(data) {
        handleDiscountSuccess(data);
      })
      .fail(function(xhr) {
        handleDiscountFailure(xhr);
      });
  });

  function handleDiscountSuccess(data) {
    const discountAmount = data.discount.discount;
    const totalAmount = $("#sumPrice").text().replace(/[^\d]/g, '');
    const total = totalAmount - discountAmount

    const formattedTotal = formatCurrency(totalAmount);
    const formattedTotal2 = formatCurrency(total);
    const formattedDiscount = formatCurrency(discountAmount);
    

    $("#discount").show();
    $('#subtotal').text(formattedTotal);
    $('#sumPrice').text(formattedTotal2);
    $('.discount-money').text(`-${formattedDiscount}`);
    $('#success-message').text(`Bạn được giảm thành công: ${formattedDiscount}`).show();

    appendDiscountInputToForm(discountAmount);
  }

  function handleDiscountFailure(xhr) {
