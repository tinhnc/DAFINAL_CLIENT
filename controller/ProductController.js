...
module.exports = {
  ...
  index: async (req, res, next) => {
    ...
    let size = await Product.countDocuments({});
  ...
  getProduct: async (req, res, next) => {
    ...
    const product = utils.mutipleMongooseToObject(new Array(productChoose));
    const totalsQuantity = product[0].quantity;

    console.log(product);
    console.log("Tổng số lượng:", totalsQuantity);
  ...
  searchProduct: async (req, res, next) => {
    ...
    if (flag) {
      products = await Product.find({
        ...
        .skip(perPage * (page - 1))
        .limit(perPage);
      size = await Product.countDocuments({
        name: { $regex: name, $options: "i" },
        price: { $gte: minPrice, $lte: maxPrice },
      });
